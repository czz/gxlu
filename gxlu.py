...
parser.add_argument("-f", "--filename",
                    help="Name of file with a list of emails")
parser.add_argument("-t", "--threads", help="number of threads, default 4")
parser.add_argument("-o", "--out", help="Name of output file")
parser.add_argument(
    "-i", "--invalid", help="if used with --out will give a file invalid.outputfilename else will print in std out", action="store_true")
...
if args.filename:
        ...
        for i, line in enumerate(fp, start=1):
            ...
            if (i % threads == 0):
                ...
                if args.out:
                    ...
                    if args.invalid:
                        invalid_results = [email for email in emails if email not in results]
                    if args.out:
                        write_to_file(invalid, invalid_results)
                    else:
                        write_to_stdout(invalid_results, tag='not valid')
    ...
    if args.invalid and not args.out:
        invalid.close()
